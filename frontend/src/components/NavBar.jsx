  ...
  return (
    <nav className="flex max-w-[1100px] dark:bg-[#313131] rounded-sm justify-between w-full bg-slate-200 px-6 py-4 ">
      ...
      <div className="flex items-center gap-4">
        <Link
          to="/CreatePage"
          className="dark:bg-[#575757] bg-[#cfcfcf] rounded-md p-1"
        >
          <CiSquarePlus size={40} color="red"></CiSquarePlus>
        </Link>
        <button
          onClick={toggleColorMode}
          className="rounded-md dark:bg-[#575757] p-2 bg-[#cfcfcf]"
        >
          {darkMode === "light" ? (
            <LuSun color="white" size={30}></LuSun>
          ) : (
            <IoMoon size={30}></IoMoon>
          )}
        </button>
      </div>
    </nav>
